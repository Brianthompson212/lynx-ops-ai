...
On the backend service, set `COACH_USERNAME` and `COACH_PASSWORD` in Render's Environment settings. Both must be set to override the built-in coach login. With both unset or empty, the built-in Lynxcoach login uses a salted password hash. Set COACH_LOGIN_DISABLED=true to disable all configured coach access. A partial override disables this login. Keep the password out of source control. Deploy the backend and frontend updates, then use Coach Login with the configured username and password. This account has coach access only and does not require a stored invitation. Changing these settings does not fix persistent storage for registered accounts.
